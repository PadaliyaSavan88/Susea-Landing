...
function TrendChart() {
  return (
    <svg viewBox="0 0 660 170" width="100%" height="170" preserveAspectRatio="none" style={{ marginTop: 6, display: 'block' }}>
      <defs>
      ...
      <g fontFamily="JetBrains Mono, monospace" fontSize="10" fill="#9AA8BE">
        <text x="44" y="22" textAnchor="end">$1,600</text>
        <text x="44" y="65" textAnchor="end">$1,450</text>
        <text x="44" y="108" textAnchor="end">$1,300</text>
      </g>
      <g stroke="#EDF1F7" strokeWidth="1">
        <line x1="50" y1="18" x2="650" y2="18" /><line x1="50" y1="61" x2="650" y2="61" />
        <line x1="50" y1="104" x2="650" y2="104" /><line x1="50" y1="125" x2="650" y2="125" stroke="#D2DBE8" />
      </g>
      <path d="M50,90 L80,82 L110,86 L140,70 L170,74 L200,60 L230,68 L260,52 L290,58 L320,42 L350,48 L380,38 L410,46 L440,34 L470,42 L500,30 L530,38 L560,28 L590,34 L620,24 L650,30 L650,125 L50,125 Z" fill="url(#gA)" />
      <path d="M50,90 L80,82 L110,86 L140,70 L170,74 L200,60 L230,68 L260,52 L290,58 L320,42 L350,48 L380,38 L410,46 L440,34 L470,42 L500,30 L530,38 L560,28 L590,34 L620,24 L650,30" fill="none" stroke="#4A82D9" strokeWidth="2" />
      <path d="M50,100 L80,96 L110,98 L140,92 L170,96 L200,88 L230,92 L260,82 L290,88 L320,80 L350,86 L380,78 L410,84 L440,74 L470,80 L500,72 L530,78 L560,70 L590,74 L620,68 L650,72" fill="none" stroke="#F07020" strokeWidth="1.6" strokeDasharray="4 3" />
      <g fontFamily="JetBrains Mono, monospace" fontSize="10" fill="#9AA8BE">
        <text x="50" y="148">Oct 26</text><text x="200" y="148">Nov 02</text><text x="350" y="148">Nov 09</text>
        <text x="500" y="148">Nov 16</text><text x="650" y="148" textAnchor="end">Nov 23</text>
      </g>
      <g fontFamily="Plus Jakarta Sans, sans-serif" fontSize="11" fill="#41506A">
        <rect x="50" y="160" width="8" height="8" rx="1.5" fill="#4A82D9" />
        <text x="64" y="168">INNSA → AEJEA spot</text>
        <rect x="210" y="160" width="8" height="8" rx="1.5" fill="#F07020" />
        <text x="224" y="168">Global FBX ocean index</text>
      </g>
    </svg>
  )
}
...
function TradeMap() {
  const dots = useMemo(() => {
    const blobs = [[200, 140, 130, 70, .5], [320, 250, 80, 70, .5], [470, 150, 130, 80, .55], [520, 260, 90, 90, .5], [700, 200, 140, 90, .6], [900, 200, 130, 80, .55], [1020, 320, 80, 50, .5]]
    const out = []
    for (let x = 20; x < 1180; x += 15)
      for (let y = 20; y < 400; y += 15)
        for (const [cx, cy, rx, ry, d] of blobs) {
          const dx = (x - cx) / rx, dy = (y - cy) / ry
          if (dx * dx + dy * dy < 1 && Math.random() < d) { out.push([x, y]); break }
        }
    return out
  }, [])
  const dests = [[540, 190, 16, 'AEJEA', 500, 180], [420, 180, 14, 'EGPSD', 380, 170], [320, 260, 14, 'ZADUR', 280, 250], [920, 170, 14, 'SGSIN', 930, 165], [820, 300, 12, 'IDJKT', 830, 296], [460, 120, 14, 'NLRTM', 470, 115], [1040, 320, 12, 'AUSYD', 990, 335], [880, 210, 12, 'CNSHA', 890, 205]]
  return (
    <svg className="map-svg" viewBox="0 0 1200 420" preserveAspectRatio="xMidYMid meet" style={{ width: '100%', height: 'auto' }}>
      <defs>
        <radialGradient id="port" cx="50%" cy="50%" r="50%"><stop offset="0" stopColor="#F07020" stopOpacity="1" /><stop offset="1" stopColor="#F07020" stopOpacity="0" /></radialGradient>
        <radialGradient id="portB" cx="50%" cy="50%" r="50%"><stop offset="0" stopColor="#4A82D9" stopOpacity="1" /><stop offset="1" stopColor="#4A82D9" stopOpacity="0" /></radialGradient>
        <linearGradient id="route" x1="0" x2="1" y1="0" y2="0"><stop offset="0" stopColor="#4A82D9" stopOpacity=".15" /><stop offset=".5" stopColor="#4A82D9" stopOpacity=".85" /><stop offset="1" stopColor="#F07020" stopOpacity=".85" /></linearGradient>
      </defs>
      <g>{dots.map(([x, y], i) => <circle key={i} cx={x} cy={y} r="1.3" fill="#C7D2E0" />)}</g>
      <g fill="none" stroke="url(#route)" strokeWidth="1.4">
        <path d="M700 215 Q 620 140 540 190" /><path d="M700 215 Q 800 160 920 170" /><path d="M700 215 Q 760 270 820 300" />
        <path d="M700 215 Q 540 250 320 260" /><path d="M700 215 Q 580 180 420 180" /><path d="M700 215 Q 760 200 880 210" />
      </g>
      <g fill="none" stroke="#4A82D9" strokeOpacity=".3" strokeDasharray="3 3" strokeWidth="1">
        <path d="M700 215 Q 600 100 460 120" /><path d="M700 215 Q 900 280 1040 320" />
      </g>
      <g><circle cx="700" cy="215" r="22" fill="url(#portB)" /><circle cx="700" cy="215" r="4" fill="#1E5BC6" />
        <text x="710" y="208" fontFamily="JetBrains Mono" fontSize="10" fill="#41506A">INNSA · Nhava Sheva</text></g>
      <g fontFamily="JetBrains Mono" fontSize="10" fill="#6B7C96">
        {dests.map(([cx, cy, r, code, tx, ty], i) => (
          <g key={i}><circle cx={cx} cy={cy} r={r} fill="url(#port)" /><circle cx={cx} cy={cy} r="3" fill="#C2540F" /><text x={tx} y={ty}>{code}</text></g>
        ))}
      </g>
    </svg>
  )
...
const TABS = ['Pricing', 'Schedules', 'Surcharges', 'Market trends', 'Saved lanes']
const CTNS = ["20' GP", "40' GP", "40' HC", "40' RF", 'LCL']
const RATES = [
  ['MA', 'Maritime A', '$1,420', '7d', '14d', 'Dec 02', 'BEST', 'tag-best', true],
  ['OC', 'Oceanic Co', '$1,475', '8d', '14d', 'Nov 30', '', '', false],
  ['PA', 'Pacific Line', '$1,510', '6d', '10d', 'Dec 04', 'FAST', 'tag-fast', false],
  ['EV', 'Eastern Ven.', '$1,540', '10d', '21d', 'Dec 02', 'FREE TIME', 'tag-eco', false],
  ['SR', 'Sealink Roy.', '$1,612', '8d', '10d', 'Nov 28', '', '', false],
  ['HN', 'Harbor Nine', '$1,684', '9d', '14d', 'Dec 06', '', '', false],
]
const SURCHARGES = [['BAF — Bunker Adj.', '$84.00'], ['CAF — Currency Adj.', '$36.00'], ['THC origin (INNSA)', '$62.00'], ['THC destination (AEJEA)', '$78.00'], ['LSS — Low Sulphur', '$28.00'], ['ISPS / Security', '$12.00'], ['Documentation', '$35.00']]
const LANES = [['INNSA → AEJEA', '$1,420', '▲4.2%', true], ['INMUN → NLRTM', '$2,840', '▼1.1%', false], ['INMAA → SGSIN', '$ 980', '▲2.6%', true], ['INPAV → USNYC', '$3,260', '▲0.8%', true], ['INNSA → ZADUR', '$1,710', '▼3.3%', false]]
